...
class TestNeuronEdge:
    ...
    def test_last_activated_auto(self):
        before = time.time()
...
class TestAtomicObservation:
    def test_default_logic_type(self):
        obs = AtomicObservation(raw_text="AAPL stock rose")
...
class TestWorkingMemoryBuffer:
    ...
    def test_push_and_len(self):
        ...
        buf.push(self._obs("hello"))
        assert len(buf) == 1

    def test_max_size_not_exceeded(self):
        buf = WorkingMemoryBuffer(max_size=3)
        for i in range(10):
            buf.push(self._obs(f"observation {i}"))
        assert len(buf) == 3

    def test_search_finds_match(self):
        buf = WorkingMemoryBuffer()
        buf.push(self._obs("AAPL stock was queried"))
        buf.push(self._obs("weather is nice today"))
        results = buf.search("AAPL stock")
        assert len(results) == 1
        assert "AAPL" in results[0].raw_text

    def test_search_empty_result(self):
        buf = WorkingMemoryBuffer()
        buf.push(self._obs("bitcoin price"))
...
class TestNeuronTree:
    ...
    def test_hop_traversal_finds_match(self):
        tree = NeuronTree()
        node = NeuronNode(label="AAPL stock")
        tree.add_node(node)
        hits = tree.hop_traversal("AAPL", hop_limit=2)
        assert len(hits) >= 1
        assert hits[0].id == node.id

    def test_hop_traversal_empty_tree(self):
        tree = NeuronTree()
        assert tree.hop_traversal("anything") == []
...
class TestDNT:
    ...
    async def test_observe_string(self):
        dnt = DNT()
        await dnt.observe("User asked about AAPL")
    ...
    async def test_observe_dict(self):
        dnt = DNT()
        await dnt.observe({"text": "User asked about portfolio", "source": "chat"})
        assert dnt.stats().observe_count == 1
    ...
    @pytest.mark.asyncio
    async def test_stats_returns(self):
        dnt = DNT()
        stats = dnt.stats()
